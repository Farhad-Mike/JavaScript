...
eventSource.onmessage;  // For new EventSource(url) Сервер может указать другой тип события с помощью event: ... в начале сообщения.
eventSource.onerror;    // For new EventSource(url) Сервер может указать другой тип события с помощью event: ... в начале сообщения.
eventSource.onopen;     // For new EventSource(url) Сервер может указать другой тип события с помощью event: ... в начале сообщения.
storage                 // Когда обновляются данные в localStorage или sessionStorage, генерируется событие storage 
success                 // база данных (indexedDB) готова к работе, готов «объект базы данных» (openRequest.result)
error                   // не удалось открыть базу данных (indexedDB).
upgradeneeded           // база (indexedDB) открыта, но её схема устарела. Если локальная версия базы данных меньше, чем версия, определённая в open, то сработает специальное событие upgradeneeded, и мы сможем сравнить версии и обновить структуры данных по мере необходимости. Это событие также сработает, если базы данных ещё не существует, так что в этом обработчике мы можем выполнить инициализацию.
versionchange           // Срабатывает при обновлении версии DataBase.
blocked                 // DataBase не было закрыто после срабатывания на нём db.onversionchange
...
event.key           // Для события onstorage. ключ, который обновился (null, если вызван .clear()).
event.oldValue      // Для события onstorage. старое значение (null, если ключ добавлен впервые).
event.newValue      // Для события onstorage. новое значение (null, если ключ был удалён).
event.url           // Для события onstorage. url документа, где произошло обновление.
event.storageArea   // Для события onstorage. объект localStorage или sessionStorage, где произошло обновление.
event.close();      // Close DataBase. use it for update database version.
